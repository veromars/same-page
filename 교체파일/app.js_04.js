...
const MATCHED_PROFILES = [
  { id: 4, name: "여름", image: MOCK_PROFILES[4].image, isNew: true },
  { id: 5, name: "수아", image: MOCK_PROFILES[5].image, isNew: false },
  { id: 6, name: "지안", image: MOCK_PROFILES[6].image, isNew: false },
  { id: 7, name: "하늘", image: MOCK_PROFILES[7].image, isNew: false }
];
...
(function () {
  ...
  window.openMatchIntroModal = function(profileId, isQurated = false) {
    ...
    amc.innerHTML = `
      <div class="match-intro-modal fade-in">
        ...
        <div class="match-intro-pm">
          ...
          ${!isQurated ? `
          <div class="match-options fade-in" id="regular-match-options" style="animation-delay: 1s; display: flex; flex-direction: column; margin-bottom: 12px; margin-top: 16px; gap: 8px; padding: 0 4px; position: relative; z-index: 10; overflow: visible;">
            <button id="btn-meetup-${match.id}" style="display:block; width:100%; padding:16px 20px; border-radius:14px; border:1.5px solid #C89FDB; background:white; color:#9B72CC; font-size:15px; font-family:'Pretendard',sans-serif; font-weight:500; text-align:center; cursor:pointer; box-shadow:0 1px 4px rgba(0,0,0,0.06); pointer-events:auto; touch-action:manipulation;">
              📅 함께 참여할 수 있는 모임이 있어요
            </button>
            <button id="btn-date-${match.id}" style="display:block; width:100%; padding:16px 20px; border-radius:14px; border:1.5px solid #C89FDB; background:white; color:#9B72CC; font-size:15px; font-family:'Pretendard',sans-serif; font-weight:500; text-align:center; cursor:pointer; box-shadow:0 1px 4px rgba(0,0,0,0.06); pointer-events:auto; touch-action:manipulation;">
              ✨ 둘만의 만남을 제안해볼까요?
            </button>
            <button id="btn-chat-${match.id}" style="display:block; width:100%; padding:16px 20px; border-radius:14px; border:1.5px solid #C89FDB; background:white; color:#9B72CC; font-size:15px; font-family:'Pretendard',sans-serif; font-weight:500; text-align:center; cursor:pointer; box-shadow:0 1px 4px rgba(0,0,0,0.06); pointer-events:auto; touch-action:manipulation;">
              💬 먼저 대화를 나눠볼게요
            </button>
            ...
            <div id="meetup-recommend-panel-${match.id}" style="display:none; margin-top:8px;">
              ...
              ${MOCK_MEETUPS.filter(m => m.isRecommended).slice(0,2).map(m => `
                <div onclick="closeAnswerCard(); openMeetupDetail(${m.id})" style="background:white; border-radius:14px; border:1px solid #EEE; padding:14px 16px; margin-bottom:8px; cursor:pointer; box-shadow:0 1px 4px rgba(0,0,0,0.06); pointer-events:auto;">
              ...
              `).join('')}
              <button id="btn-more-meetups-${match.id}" style="display:block; width:100%; padding:12px; border-radius:14px; border:1px solid #EEE; background:white; color:var(--text-muted); font-size:13px; cursor:pointer; margin-top:4px; pointer-events:auto;">
                모임 더 보기 →
              </button>
            </div>

            <!-- 둘만의 만남 패널 -->
            <div id="date-options-panel-${match.id}" style="display:none; margin-top:8px; overflow:visible;">
            </div>
          </div>
          ` : ''}
    ...
    // Bind buttons via addEventListener (bypasses onclick issues)
    setTimeout(() => {
      const matchId = match.id;

      const btnMeetup = document.getElementById(`btn-meetup-${matchId}`);
      if (btnMeetup) {
        btnMeetup.addEventListener('click', function(e) {
          e.stopPropagation();
      ...
      const btnDate = document.getElementById(`btn-date-${matchId}`);
      if (btnDate) {
        btnDate.addEventListener('click', function(e) {
          ...
          panel.innerHTML = `
            ...
            <div style="display:flex; flex-wrap:wrap; gap:8px; justify-content:center; margin-bottom:12px;" id="date-type-pills-${matchId}">
              ${['☕ 카페', '🍽 식사', '🍺 술 한 잔', '💡 기타 제안'].map((opt, i) => `
                <button id="date-type-btn-${matchId}-${i}" data-type="${opt}" style="padding:10px 16px; border-radius:999px; border:1.5px solid #C89FDB; background:white; color:#9B72CC; font-size:14px; cursor:pointer; font-family:'Pretendard',sans-serif; pointer-events:auto; touch-action:manipulation;">
                  ${opt}
                </button>
              `).join('')}
            </div>
          ...
          // Bind type pill buttons
          ['☕ 카페', '🍽 식사', '🍺 술 한 잔', '💡 기타 제안'].forEach((opt, i) => {
            const typeBtn = document.getElementById(`date-type-btn-${matchId}-${i}`);
            if (typeBtn) {
              typeBtn.addEventListener('click', function(e) {
                ...
                panel.innerHTML = `
                  ...
                  <div style="display:flex; flex-wrap:wrap; gap:8px; justify-content:center; margin-bottom:12px;" id="date-time-pills-${matchId}">
                    ${['이번 주말', '다음 주말', '평일 저녁', '날짜 직접 제안'].map((date, j) => `
                      <button id="date-time-btn-${matchId}-${j}" data-date="${date}" style="padding:10px 16px; border-radius:999px; border:1.5px solid #C89FDB; background:white; color:#9B72CC; font-size:14px; cursor:pointer; font-family:'Pretendard',sans-serif; pointer-events:auto; touch-action:manipulation;">
                        ${date}
                      </button>
                    `).join('')}
                  </div>
                `;
                // Bind date pill buttons
                ['이번 주말', '다음 주말', '평일 저녁', '날짜 직접 제안'].forEach((date, j) => {
                  const dateBtn = document.getElementById(`date-time-btn-${matchId}-${j}`);
                  if (dateBtn) {
                    dateBtn.addEventListener('click', function(e) {
                      ...
                      if (input) {
                        if (date === '날짜 직접 제안') {
                          input.value = `${opt} 어떠세요? 편하신 날짜 알려주세요 😊`;
                        } else if (opt === '💡 기타 제안') {
                          input.value = `${date}에 어떤 만남이면 좋을지 제안해줄 수 있어요? 😊`;
                        } else {
                          input.value = `${date}에 ${opt} 어떠세요? 😊`;
                        }
                        input.focus();
                      }
      ...
      const btnChat = document.getElementById(`btn-chat-${matchId}`);
      if (btnChat) {
        btnChat.addEventListener('click', function(e) {
          e.stopPropagation();
          const opts = document.getElementById('regular-match-options');
      ...
      const btnMoreMeetups = document.getElementById(`btn-more-meetups-${matchId}`);
      if (btnMoreMeetups) {
        btnMoreMeetups.addEventListener('click', function(e) {
          e.stopPropagation();
          closeAnswerCard();
          switchTab('meetups');
        });
    ...
    panel.innerHTML = `
      ...
      <div style="display:flex; flex-wrap:wrap; gap:8px; justify-content:center; margin-bottom:12px;">
        ${['☕ 카페', '🍽 식사', '🍺 술 한 잔', '💡 기타 제안'].map(opt => `
          <button ontouchend="event.preventDefault(); window.selectDateType('${opt}', ${matchId});" onclick="window.selectDateType('${opt}', ${matchId})" style="padding:10px 16px; border-radius:999px; border:1.5px solid #C89FDB; background:white; color:#9B72CC; font-size:14px; cursor:pointer; font-family:'Pretendard',sans-serif; pointer-events:auto; touch-action:manipulation;">
            ${opt}
          </button>
        `).join('')}
      </div>
    ...
    panel.innerHTML = `
      ...
      <div style="display:flex; flex-wrap:wrap; gap:8px; justify-content:center; margin-bottom:12px;">
        ${['이번 주말', '다음 주말', '평일 저녁', '날짜 직접 제안'].map(date => `
          <button ontouchend="event.preventDefault(); window.selectMeetupDate('${type}', '${date}', ${matchId});" onclick="window.selectMeetupDate('${type}', '${date}', ${matchId})" style="padding:10px 16px; border-radius:999px; border:1.5px solid #C89FDB; background:white; color:#9B72CC; font-size:14px; cursor:pointer; font-family:'Pretendard',sans-serif; pointer-events:auto; touch-action:manipulation;">
            ${date}
          </button>
        `).join('')}
      </div>
    ...
    const renderChatView = () => {
      ...
      <div style="position: absolute; top:-60px; left:0; width: 100%; height: calc(100vh - 84px); background: var(--bg-color); z-index: 50; display:flex; flex-direction:column;">
        <div class="chat-header" style="position: relative; justify-content: center; padding: 12px 20px; min-height: 60px;">
          ...
          <div class="chat-header-user-info" style="display:flex; flex-direction:column; align-items:center; cursor:pointer;" onclick="openChatProfile(${chatId})">
