    ...
    amc.innerHTML = `
      <div class="match-intro-modal fade-in">
        ...
        <div class="match-intro-pm">
          ...
          ${!isQurated ? `
          <div class="match-options fade-in" id="regular-match-options" style="animation-delay: 1s; display: flex; flex-direction: column; margin-bottom: 12px; margin-top: 12px;">
            <button class="match-option-btn" onclick="closeAnswerCard(); switchTab('meetups');">
              📅 함께 참여할 수 있는 모임이 있어요
            </button>
            <button class="match-option-btn" onclick="document.getElementById('match-intro-input-field').value = '✨ 둘만의 만남을 제안해볼까요?'; document.getElementById('match-intro-input-field').focus();">
              ✨ 둘만의 만남을 제안해볼까요?
            </button>
            <button class="match-option-btn" onclick="document.getElementById('regular-match-options').style.display = 'none'; document.getElementById('match-intro-input-field').focus();">
              💬 먼저 대화를 나눠볼게요
            </button>
          </div>
          ` : ''}
